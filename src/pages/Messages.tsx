...
import { Send, Paperclip, ArrowLeft, MessageSquare, X } from "lucide-react";
...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    const loadConversation = async () => {
      try {
        ...
        setReceiverId(data.participant_id);
      } catch (err) {
        console.error(err);
        toast({
          title: "Erro",
  ...
  const sendMessage = async () => {
    ...
    const { data, error } = await supabase.from("messages").insert([messageToInsert]).select().single();
    ...
    // Marcar mensagem como enviada com check
    setSentMessageIds((prev) => [...prev, data.id]);

    await supabase
      .from("conversations")
      .update({ last_message: messageContent, last_timestamp: new Date().toISOString() })
      .eq("id", id);
  };
    ...
    <div className="bg-background min-h-screen flex flex-col">
          ...
          >
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                >
                  <p className="text-sm leading-relaxed flex items-center justify-between">
                    {msg.content}
                    {msg.sender_id === user.id && sentMessageIds.includes(msg.id) && (
                      <span className="ml-2 text-green-500 font-bold animate-pulse">✔</span>
                    )}
                  </p>
                  {msg.files?.map((f, idx) => (
                    <a
      ...
      <div className="sticky bottom-0 bg-background px-4 py-3 border-t border-border">
        <div className="flex flex-col gap-2">
          ...
          {selectedFiles.length > 0 && (
            <div className="flex gap-2 flex-wrap">
              {selectedFiles.map((file, idx) => (
                ...
                >
                  {file.name}
                  <button onClick={() => removeFile(idx)} className="ml-1 text-red-500">
                    <X size={12} />
                  </button>
                </div>
              ))}
          ...
          <div className="flex items-center gap-2 bg-card border border-border rounded-full px-4 py-4">
            <Button
              variant="ghost"
              size="icon"
              type="button"
            ...
            <textarea
              ...
              className="flex-1 resize-none border-none focus:ring-0 bg-transparent px-3 py-3 text-sm h-20 rounded-2xl"
            />
            <Button
              type="button"
              onClick={sendMessage}
